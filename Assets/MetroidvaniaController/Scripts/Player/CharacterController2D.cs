...
public class CharacterController2D : MonoBehaviour
{
	...
	private bool wasJumpingPreviously = false;
	...
	public void Move(float move, bool jump,bool jumpHeld, bool dash)
	{
		if (canMove) 
		{
			...
			if(jump && gauges.CanJump())
            {
				...
				else if (!m_Grounded && canDoubleJump && !isWallSliding)
				{
					//canDoubleJump = false; // do we want to prevent further double jumps?
			...
			{
				if (gauges.CanGlide() && jumpHeld)
				{
					gauges.OnGlide(Time.fixedDeltaTime);
				}
				else if (Vector2.Dot(m_Rigidbody2D.velocity, Vector2.up) > 0)
				{
					m_Rigidbody2D.AddForce(new Vector2(0f, -m_JumpForce/2f) * m_Rigidbody2D.mass);
				}
			}
